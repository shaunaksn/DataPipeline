# importing snowflake connector module from lambda layer
...
# function to run querry on snowflake
...
#
lambda function for snowfalake operation from lambda

def lambda_handler(event, context):
    #snowflake credentials varables declarration
    user="Pravinkumaar97"
    password="Pravin97@p"
    account="at95729.ca-central-1.aws"
    conn=sf.connect(user=user,password=password,account=account);
    
    # statement to preparing the platform of snowflake
    statement_1='use warehouse COMPUTE_WH';
    statement4="use role SYSADMIN";
    sql_query_Database_creation = "CREATE or REPLACE database PROJECT_DB;"
    statement3="use database PROJECT_DB;"
    #statement to execte the statements of platform creation
    run_query(conn,statement_1)
    run_query(conn,statement4)
    run_query(conn, sql_query_Database_creation)
    run_query(conn,statement3);
    
    #creating shemas for loading parquet files
    #file_format_schema
    ...
    #creating file format parquet parqut type at "file_format" schema
    ...
    sql_querry_dim_policy_stage_creation="CREATE OR REPLACE STAGE PROJECT_DB.External_stage.PARAQUET_STG URL = 's3://mini-project-s3-stage-bucket/dim_policy_with_header/' CREDENTIALS = (AWS_KEY_ID = 'AKIAUER7PRQAYDHANNP6' AWS_SECRET_KEY = 'FOORFCzVE58EkS0RK/kraxCXujRhYmLnFAUyrHsx')file_format = PROJECT_DB.File_Format.PARAQUET_FF;"
    ...
    sql_querry_dim_product_stage_creation="CREATE OR REPLACE STAGE PROJECT_DB.External_stage.DIM_PRODUCT_STG URL = 's3://mini-project-s3-stage-bucket/dim_product_with_headers/' CREDENTIALS = (AWS_KEY_ID = 'AKIAUER7PRQAYDHANNP6' AWS_SECRET_KEY = 'FOORFCzVE58EkS0RK/kraxCXujRhYmLnFAUyrHsx')file_format = PROJECT_DB.File_Format.PARAQUET_FF;"
    ...
    sql_querry_fact_policy_stage_creation="CREATE OR REPLACE STAGE PROJECT_DB.External_stage.FACT_POLICY_STAGE URL = 's3://mini-project-s3-stage-bucket/FACT_POLICY_202202251501/' CREDENTIALS = (AWS_KEY_ID = 'AKIAUER7PRQAYDHANNP6' AWS_SECRET_KEY = 'FOORFCzVE58EkS0RK/kraxCXujRhYmLnFAUyrHsx')file_format = PROJECT_DB.File_Format.PARAQUET_FF;"
